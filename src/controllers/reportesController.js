...
export const reporteCargasPorPeriodo = async (req, res) => {
  ...
  try {
    ...
    const costoPagado = cargas.filter((carga) => carga.estado === "pagado").reduce((sum, carga) => sum + carga.costo, 0)
    const costoDeuda = cargas.filter((carga) => carga.estado === "deuda").reduce((sum, carga) => sum + carga.costo, 0)
  ...
  try {
    ...
    const rendimientoPorTipo = await Promise.all(
      tiposCamion.map(async (tipo) => {
        ...
        // Obtener historial de precios para este tipo de camión
